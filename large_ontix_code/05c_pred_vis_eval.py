...
results_folder = "/data/horse/ws/jaew523d-large_ontix_project/results/large_disentanglix_save/tests/"
...
final_disentanglix_model_file = os.path.join(results_folder, f"large_disentanglix_final_model_{ont_from_cli}.pkl")
...
loaded_disentanglix = acx.Disentanglix.load(file_path=final_disentanglix_model_file)
loaded_disentanglix._trainer._config.save_vram = True  # Enable memory saving for prediction on holdout set

print("Predicting on holdout data ...")
result = loaded_disentanglix.predict(
	data= acx_container
)
...
# params_umap = ["assay"]
loaded_disentanglix.visualizer.show_latent_space(
	result=loaded_disentanglix.result,
...
loaded_disentanglix.result.get_latent_df(epoch=-1, split='test').to_parquet(os.path.join(results_folder, f"holdout_latent_{ont_from_cli}.parquet"))

# Save plots
path_plots = os.path.join(results_folder, f"large_disentanglix_holdout_{ont_from_cli}_plots/")
# Create directory if it does not exist
os.makedirs(path_plots, exist_ok=True)
loaded_disentanglix.visualizer.save_plots(
	path=path_plots, which ='all', format ='png'
)

# Save evaluation results
path_eval = os.path.join(results_folder, f"large_disentanglix_holdout_{ont_from_cli}_evaluation.csv")
loaded_disentanglix.result.embedding_evaluation.to_csv(path_eval, index=False)