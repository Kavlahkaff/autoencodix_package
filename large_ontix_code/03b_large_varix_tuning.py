...
results_folder = "/data/horse/ws/jaew523d-large_ontix_project/results/large_varix_save/first_run_e250/"
...
dim_from_cli = sys.argv[1]  # "Dim10", "Dim24", "Dim32"
...
def syne_trainer(
	...
	scconfig = VarixConfig(
		...
		n_layers= n_layers,
		latent_dim= int(dim_name[3:]), # Extract the dimension from the string (e.g., "Dim32" -> 32)
		save_memory=False,
	...
	avg_celltype_performance = varix.result.embedding_evaluation.loc[
		(varix.result.embedding_evaluation.score_split == "valid") &
		(varix.result.embedding_evaluation.CLINIC_PARAM.str.startswith("Task")),
		"value"
	].mean()
	## Average of all other tasks
	avg_other_performance = varix.result.embedding_evaluation.loc[
		(varix.result.embedding_evaluation.score_split == "valid") &
		(~varix.result.embedding_evaluation.CLINIC_PARAM.str.startswith("Task")),
		"value"
	].mean()
	...
	valid_recon_loss = float(varix.result.sub_losses.get("recon_loss").get(epoch=-1, split="valid"))
 
	# Variance across tasks
	var_celltype_performance = varix.result.embedding_evaluation.loc[
		(varix.result.embedding_evaluation.score_split == "valid") &
		(varix.result.embedding_evaluation.CLINIC_PARAM.str.startswith("Task")),
		"value"
	].var()
	var_other_performance = varix.result.embedding_evaluation.loc[
		(varix.result.embedding_evaluation.score_split == "valid") &
		(~varix.result.embedding_evaluation.CLINIC_PARAM.str.startswith("Task")),
		"value"
	].var()
...
config_space = {
	...
	"n_layers": randint(2, 2), # Varix less complex encoder matching overall Ontix number of parameters
}
...
with open(os.path.join(results_folder, f"large_varix_{dim_from_cli}_tuning.pkl"), "wb") as f:
