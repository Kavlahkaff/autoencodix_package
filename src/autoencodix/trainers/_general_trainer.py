...
class GeneralTrainer(BaseTrainer):
    ...
    def train(self, epochs_overwrite=None) -> Result:
        ...
        with self._fabric.autocast():
            ...
            for epoch in range(epochs):
                ...
                self._log_losses(epoch=epoch, split="train", total_loss=epoch_loss, sub_losses=epoch_sub_losses, should_checkpoint=should_checkpoint)

                if self._validset:
                    valid_loss, valid_sub_losses = self._validate_epoch(
                        should_checkpoint=should_checkpoint, epoch=epoch
                    )
                    self._log_losses(epoch=epoch, split="valid", total_loss=valid_loss, sub_losses=valid_sub_losses, should_checkpoint=should_checkpoint)
    ...
    def _log_losses(
        self, epoch: int, split: str, total_loss: float, sub_losses: Dict[str, float], should_checkpoint: bool = False
    ) -> None:
